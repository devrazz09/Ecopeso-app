...
interface DonatioCardProps {
    title: DonationTitle;
    popUpModal? : (status: boolean) => void;
}


export default function DonationCard({ title, popUpModal } : DonatioCardProps ){
    return(
        <div className="w-123.5 h-128.25 flex flex-col justify-center items-center gap-6 bg-white rounded-3xl shadow-[0px_0px_5px_1px_rgba(162,162,162,0.8)] font-mono">
            ...
            <button className="bg-primary text-white px-6 py-4 rounded-lg" onClick={() => popUpModal && popUpModal(true)}>Donate Now</button>
        </div>
    )
}